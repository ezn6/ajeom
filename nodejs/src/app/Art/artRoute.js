...
// 클라이언트에서 넘어온 파일에 대한 정보가 req.file에 FILE 객체로 저장되어 있습니다.
    ...
    //내이미지함 삭제
    app.patch('/app/users/:userId/myimg/:myimgId', jwtMiddleware, art.delImg);
    ...
    //작품상세페이지
    app.get('/app/users/:userId/artworks/:artId',jwtMiddleware, art.artDetail);

    //보관함 삭제
    app.patch('/app/users/:userId/storage/:storageId',jwtMiddleware, art.delStorage);

    //보관함 좋아요
    app.post('/app/users/:userId/storage/:storageId/like',jwtMiddleware, art.likeStorage);
    //보관함 좋아요 취소
    app.patch('/app/users/:userId/storage/:storageId/like', jwtMiddleware, art.delLikeStorage);

    //보관함 저장(옅보기저장)
    app.post('/app/users/:userId/storage/:storageId/save',jwtMiddleware, art.saveStorage);
    //보관함 저장 취소
    app.patch('/app/users/:userId/storage/:storageId/save', jwtMiddleware, art.delSaveStorage);
